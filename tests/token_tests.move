...
module token::token_tests {
    ...
    fun mint_test_tokens(scenario: &mut ts::Scenario): (TreasuryCap<BITS>, Coin<BITS>) {
        ts::next_tx(scenario, ADMIN);
        let ctx = ts::ctx(scenario);
        let mut treasury_cap = bits::test_init(ctx);
    ...
    fun test_token_initialization() {
        ...
        {
            let mut treasury_cap = bits::test_init(ts::ctx(&mut scenario));
    ...
    fun test_token_transfer() {
        ...
        {
            let received_coins = ts::take_from_sender<Coin<BITS>>(&scenario);
